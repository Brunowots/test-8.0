...
void ProtocolGame::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	std::cout << "ProtocolGame version: " << version << std::endl;
	std::cout << "Min: " << g_config.getNumber(ConfigManager::VERSION_MIN) << std::endl;
	std::cout << "Max: " << g_config.getNumber(ConfigManager::VERSION_MAX) << std::endl;
...
{
	// Protocol 8.0 - no challenge timestamp/random sent
}
...
void ProtocolGame::parseAutoWalk(NetworkMessage& msg)
{
	...
	std::forward_list<Direction> listDir(path.begin(), path.end());

	addGameTask(&Game::playerAutoWalk, player->getID(), std::move(listDir));
}
...
void ProtocolGame::sendOutfitWindow()
{
	...
	for (const ProtocolOutfit& outfit : protocolOutfits) {
		msg.addByte(static_cast<uint8_t>(outfit.lookType));
...
void ProtocolGame::AddPlayerStats(NetworkMessage& msg)
{
	...
	msg.add<uint16_t>(std::min<int32_t>(player->getFreeCapacity(), std::numeric_limits<int16_t>::max()));

	msg.add<uint32_t>(std::min<uint32_t>(player->getExperience(), 0x7FFFFFFF));

	msg.addByte(std::min<int32_t>(player->getLevel(), std::numeric_limits<uint8_t>::max()));
